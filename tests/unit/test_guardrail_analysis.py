...
from andela_mcp.chat import TOOL_MIN_PRINCIPAL, _principal_can_call, build_function_tools
...
def test_tool_visibility_by_principal():
    ...
    async def run_tests():
        ...
        anonymous_tools = await build_function_tools(
            {"remote-mcp": client}, [], principal="anonymous"
        )
        ...
        assert anonymous_names == expected_anonymous, (
            f"Anonymous mismatch: got {anonymous_names}, expected {expected_anonymous}"
        )

        # Test customer principal
        customer_tools = await build_function_tools(
            {"remote-mcp": client}, [], principal="customer"
        )
        ...
        assert customer_names == expected_anonymous, (
            f"Customer mismatch: got {customer_names}, expected {expected_anonymous}"
        )
        ...
        assert staff_names == expected_staff, (
            f"Staff mismatch: got {staff_names}, expected {expected_staff}"
        )
...
def test_principal_hierarchy():
    ...
    # Test customer principal accessing customer-level tools
    # (none in current mapping, but let's test staff tool)
...
def test_current_tool_mappings():
    ...
    for tool in customer_service_tools:
        ...
        assert required in ["anonymous", "customer"], (
            f"Customer service tool {tool} requires {required}"
        )

    # Verify that administrative tools are properly restricted
    admin_tools = ["remote-mcp__get_customer"]
