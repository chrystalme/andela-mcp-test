...
from andela_mcp.guardrails.technical_support import NON_SUPPORT_PATTERNS, TECHNICAL_SUPPORT_TOPICS
...
def analyze_current_logic():
    ...
    test_cases = [
        ...
        (
            ...
            False,
            "Weather query - no support keywords, no non-pattern matches",
        ),
        (
            "What is 2+2?",
            False,
            "Math query - no support keywords, no non-pattern matches (doesn't contain math words)",
        ),
        (
            "Who won the football match?",
            False,
            "Sports query - but sport pattern should catch this",
        ),
        ("Let's talk about movies", False, "Movie discussion - movie pattern should catch this"),
        (
            "Translate hello to Spanish",
            False,
            "Translation - should be caught by translate pattern",
        ),
        ("Recommend a good book", False, "Book recommendation - should be caught by book pattern"),
        (
            "Explain quantum physics",
            False,
            "Physics explanation - should be caught by physics pattern",
        ),
        ("", False, "Empty string"),
...
def test_improved_logic():
    ...
    def is_technical_support_related_improved(text: str) -> bool:
        ...
        # If there are strong non-support indicators and weak/no support indicators,
        # likely not support
        if non_support_matches > 0 and support_score == 0:
            return False
