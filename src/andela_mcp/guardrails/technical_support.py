...
def is_technical_support_related(text: str) -> bool:
    ...
    if not text or not text.strip():
        ...
        non_support_matches = sum(
            1 for pattern in NON_SUPPORT_PATTERNS if re.search(pattern, text_lower)
        )
    ...
    if non_support_matches > 0:
        ...
        MIN_SUPPORT_KEYWORDS_TO_OVERRIDE_WEAK_NON_SUPPORT = 2
        return support_score >= MIN_SUPPORT_KEYWORDS_TO_OVERRIDE_WEAK_NON_SUPPORT
    ...
    SOCIAL_WORD_THRESHOLD = 0.5
    if (
        total_words > 0 and social_word_count / total_words > SOCIAL_WORD_THRESHOLD
    ):  # At least half social words
        return True

    # Very short messages that are just pleasantries
    VERY_SHORT_MESSAGE_LENGTH = 2
    return bool(
        len(text.strip()) <= VERY_SHORT_MESSAGE_LENGTH
        and text.strip() in {"hi", "hello", "thanks", "thx", "pls", "sry"}
    )
...
async def technical_support_input_guardrail(
    ...
    """
    # Extract text from input (can be string or list of message items)
    ...
    if not is_technical_support_related(text):
        return GuardrailFunctionOutput(
            output_info={"reason": "Input is not related to technical support or customer service"},
            tripwire_triggered=True,
        )

    return GuardrailFunctionOutput(
        output_info={"reason": "Input is related to technical support/customer service"},
...
async def technical_support_output_guardrail(
    ...
    MOSTLY_SOCIAL_THRESHOLD = 0.7
    if (
        total_words > 0 and social_word_count / total_words > MOSTLY_SOCIAL_THRESHOLD
    ):  # Mostly social
        return GuardrailFunctionOutput(
    ...
    if not is_technical_support_related(output):
        return GuardrailFunctionOutput(
            output_info={"reason": "Output contains non-technical support content"},
            tripwire_triggered=True,
        )

    return GuardrailFunctionOutput(
        output_info={"reason": "Output is within technical support bounds"},
        tripwire_triggered=False,
    )
